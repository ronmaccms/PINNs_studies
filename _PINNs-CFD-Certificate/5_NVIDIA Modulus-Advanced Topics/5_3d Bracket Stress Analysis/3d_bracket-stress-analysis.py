...
def run(cfg: ModulusConfig) -> None :
    ...
    nu = 0.3  # Poisson's ratio, a material property that describes the ratio of lateral strain to axial strain.
    E = 100e9  # Young's modulus in Pascals (Pa), a material property indicating the stiffness of the material.
    # Calculate the first Lamé parameter (lambda_) using Young's modulus (E) and Poisson's ratio (nu)
    lambda_ = nu * E / ((1 + nu) * (1 - 2 * nu))
    # Calculate the second Lamé parameter (mu), also known as the shear modulus
    mu = E / (2 * (1 + nu))
    # Scale the shear modulus to create a normalized version (mu_c) for simulation
    mu_c = 0.01 * mu 
    # Normalize lambda_ and mu by mu_c to use them in the simulation
    lambda_ = lambda_ / mu_c
    mu = mu / mu_c
    # Set characteristic length and displacement for the simulation, used for normalization
    characteristic_length = 1.0  # Reference length in meters.
    characteristic_displacement = 1e-4  # Reference displacement in meters.
    # Calculate a normalization factor for stress (sigma_normalization) using the characteristic length and displacement
    sigma_normalization = characteristic_length / (characteristic_displacement * mu_c)
    # Define the traction force (T) applied, scaled by the normalization factor
    T = -4e4 * sigma_normalization
    # Initialize a linear elasticity model using the normalized Lamé parameters and a 3-dimensional space
